...
- Publisher errors retain whether they came from transport rejection, structured HTTP, unstructured HTTP, or local validation.
- A transport-rejected `RUNTIME_GATEWAY_UNAVAILABLE` or unstructured proxy-generated `RUNTIME_GATEWAY_HTTP_502` is presented as `Offline`.
- A structured HTTP error response from the Gateway retains its application error code and message, including when its HTTP status is 502, its code is omitted, or its explicit code is `RUNTIME_GATEWAY_HTTP_502`.
...
- Review RED: 2 of 14 App integration tests failed because structured 502 payloads with no code or the fallback-shaped code were classified as Offline; GREEN passed the provenance-aware focused gate with 3 files and 36 tests.

## Verification

- Focused gate: 3 files, 34 tests passed.
- Review-fix focused gate: 3 files, 36 tests passed.
- Full unit gate: 171 files, 2,190 tests passed.
...
- `fix: retain gateway error response provenance` (exact SHA reported in the handoff)
...
- Offline classification now depends on publisher provenance rather than an ambiguous fallback error code.
