...
test('builds, saves, reloads, and edits a reusable workcell', async ({ page }) => {
  ...
  const axisTreeItem = page.locator('[data-scene-entity-id="linear-axis:active"]')
  const axisTreeRow = axisTreeItem.locator(':scope > .scene-tree-row')
  await axisTreeRow.click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Set Carriage' }).click()
  const carriageDialog = page.getByRole('dialog', { name: 'Choose carriage' })
  await expect(carriageDialog.getByRole('button', { name: 'Set Fixture A as carriage' }))
    .toBeFocused()
  await expect(carriageDialog.getByRole('button', { name: 'Set Cup as carriage' }))
    .toHaveCount(0)
  await carriageDialog.getByRole('button', { name: 'Set Fixture A as carriage' }).click()
  await expect.poll(async () => entity(await snapshot(page), 'linear-axis:active').carriageEntityId)
    .toBe('group:fixture-a')
  await axisTreeRow.click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Clear Carriage' }).click()
  await expect.poll(async () => entity(await snapshot(page), 'linear-axis:active').carriageEntityId)
    .toBeNull()
