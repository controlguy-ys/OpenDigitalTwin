...
2. Classify changed paths and choose the narrowest closed `npm run --silent verify:codex -- --scope <scope> --json` profile: `guidance` for repository guidance only, `project-v5` for Project V5 changes, `gateway` for Runtime Gateway changes, `ui` for browser/UI changes, and `full` when changes cross profiles or the scope is not closed.
