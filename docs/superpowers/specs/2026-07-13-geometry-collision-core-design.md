...
- one rendered Robot;
- Robot Links, Tool/Gripper, held Object, legacy Equipment, and imported Object
  Instances participate through one runtime Entity contract;
...
Each oriented box also produces a World AABB. Sweep-and-prune on the X axis
rejects non-overlapping candidates, followed by Y/Z checks. Pair policy is
applied before narrow phase.

The runtime never tests an unconstrained all-to-all scene. Default categories:

- Robot Link or Tool against Equipment/Object;
- held Object against Equipment/Object;
...
Link/Tool boxes, static Entity boxes, sequence records, and policy. It computes
FK and geometry queries without Three.js scene objects and returns findings plus
summary statistics.
...
The project format advances to schema V2. V2 keeps existing Box center/half
extents and adds:
...
- Default active proxies: at most 7 Robot Link boxes, 1 Tool box, and 256
  external boxes.
...
4. Imported Object collider metadata participates in current-pose and sequence
   validation.
5. OBB collision and proxy near-miss results are deterministic under Entity
   input order changes.
6. Pair ignore/restore takes effect without changing scene transforms.
7. Collision never changes Object/Robot pose; it only reports, highlights,
   logs, and optionally pauses playback.
8. Grasp detection works without Physics/RigidBody and held Objects remain TCP
   children.
9. Preview and Validate sequence scans return stable sample counts and finding
   times independent of render FPS.
10. Collision findings can be navigated and exported as JSON/CSV.

### Persistence and compatibility

11. Existing V1 `.wdtwin` projects import with identical visible placement and
    one generated Box per prior collider.
12. V2 export/import preserves Compound Boxes, policy, ignored pairs, Robot,
    Objects, frames, Poses, and OPC UA settings.
13. Invalid or oversized collision data is rejected atomically.

### Performance and quality

14. Current-pose queries never exceed the configured 10 Hz scheduler cap.
15. Sequence scan runs in a Worker, supports cancellation, and enforces sample
    and finding caps.
16. A reference fixture with 7 Link boxes, 1 Tool box, and 50 external boxes
    completes a 1,000-sample Validate run without blocking browser animation.
17. Scene telemetry exposes current Entity/Box/candidate/narrow-phase/finding
    counts.
18. Full unit/integration suite, CAD validation, production build, Playwright
    project round-trip, Web-only Docker smoke, and OPC UA-profile smoke pass.

### Product and safety language

19. UI and reports identify results as `Geometry Proxy Collision` and
    `Approximate Clearance`.
20. Documentation explicitly states that results are not physics, RobotWare,
