...
export interface LegacyStoredProjectPointerV0 {
  readonly key: 'active'
  readonly revisionId: string
}
```

Add `projectRevisions` keyed by `revisionId` and indexed by `projectId`, `projectSourceBlobs` keyed by `key`, and `projectPointers` keyed by `key`. Retain the v1 `projects` table for adoption and recovery during this compatibility cycle. A revision row intentionally has no mutable parent pointer and contains no source `ArrayBuffer`: Robot source projections omit `sourceBytes` while retaining `id === sha256`; STEP Object projections replace `sourceBytes` with `sourceSha256`; primitive branches are unchanged. Raw Mechanics Manifest bytes are not stored. Content addressing is deliberately namespace-local, so equal bytes create one `robot:<sha256>` Blob and one `object:<sha256>` Blob when used in both roles, but only one Blob within either namespace.

Every native V1 pointer in this schema has a required commit token. Decode the
tokenless `LegacyStoredProjectPointerV0` only inside one-time adoption and write
a normalized token-bearing stable pointer before exposing repository operations;
normal commit/finalize/GC code never accepts the legacy shape.
...
`finalizePublication(token)` atomically changes only the matching publishing pointer to stable and retains that token on the stable pointer. If another tab already finalized the same token/target, finalization is an idempotent success; a stable different token/target fails closed. `compensatePublication(token)` restores the retained previous pointer only while the matching pointer is still publishing and never rolls back an already stable target. `garbageCollect()` opens one Dexie read-write transaction spanning pointer, revision, and Blob tables; inside that same transaction it rereads the pointer, derives the stable/publishing mark set, removes every other revision, then removes every Blob unreachable from the retained projections. This transaction boundary serializes against a second-tab `commitPreparedRevision()` so GC cannot mark A, interleave commit B, and sweep B. Run it after finalization and at startup. Cleanup-retry IDs for in-memory Three.js disposal do not pin DB revisions or Blobs. A cleanup failure never rolls back a commit: emit one bounded diagnostic and retry idempotently.

Legacy adoption validates/migrates/stages the old row first and uses the same atomic publishing pointer. Legacy cleanup is forbidden until the matching V3 pointer is stable, the new runtime plus verified handles are active, and a fresh DB-only hydration/integrity check proves that exact active V3 projection and every Blob can reopen without legacy data. Only after all three conditions may the legacy full-byte Project row and obsolete `robot-sim-object-assets` (`assets`/`instances`) plus `robot-sim-equipment` (`equipment`/`scene`) rows be cleared idempotently. A finalization or activation failure retains every legacy row for recovery. From the first successfully activated V3 publication onward, normal hydration and commands never read or write those feature databases. Cleanup failure retains their rows, emits one bounded retry diagnostic, and never rolls back or makes them authoritative; subsequent startup retries only after repeating the same stable-pointer/activation/reopen proof. Tests inject finalization, activation, integrity-proof, and cleanup failures: the first three retain all rows, while cleanup failure retries and a later successful proof clears each obsolete row exactly once. This policy prevents indefinite duplicate STEP bytes while preserving recoverability during adoption.

- [ ] **Step 5: Run GREEN, reopen, and concurrency tests**

Run: `npm run test:run -- src/features/project/project-db.test.ts src/features/project/project-revision-repository.test.ts`

Expected: PASS for reopen persistence, byte-free immutable rows, namespace-local Blob de-duplication, stale-writer rejection, legacy adoption idempotence, startup orphan cleanup, exact quota mapping, newly verified Robot/Object byte identity, zero rehash/rewrite on metadata edits, bounded revision retention, tuple ownership, and input/public-read buffer isolation.
