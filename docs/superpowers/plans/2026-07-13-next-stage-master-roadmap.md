...
**Goal:** Complete the lightweight Web Digital Twin vertical slice with deterministic one-to-seven-source Robot assembly import, Project-owned Simulation Jobs, Box/Cylinder Object creation, OPC UA Object XYZRPY binding with deterministic smoothing, and a four-mode engineering workspace.
...
- Robot Import accepts one through seven unique STEP/STP source files and must resolve exactly seven Links. The seven-file limit applies only to Robot Import.
- Each imported external Object remains one whole STEP/STP source Asset. Box and Cylinder are generated Assets and never receive fake STEP bytes.
...
- Fixed World/Base/MCP/Flange/TCP coordinate frames and manual Robot/Object transforms.
...
- [ ] **Step 1: Amend the V3 aggregate before code changes**
  ...
  Freeze `WorkcellProjectSnapshotV3` as one aggregate containing Robot source Assets and Link references, Simulation Jobs and `activeJobId`, `step | box | cylinder` Object Asset sources, Object manual fallback transforms, and external transform profile bindings. Keep workspace mode and live OPC telemetry outside the archive.
...
| WS5 | [OPC UA Transform and Smoothing](./2026-07-13-opcua-transform-smoothing.md) | Connector transform profiles, browser binding, ownership, trajectory | WS1 + WS6 Stage A |
...
- WS5 owns raw/accepted/effective external transforms and source ownership. Rendering and collision read the same effective transform selector.
- WS6 owns navigation and presentation. It must not duplicate domain state or make mode switching mutate source, Job, Project, or scene state.
...
- Accept one through seven unique STEP/STP source files and parse/hash each unique source exactly once.
...
- Only the active Job can play or validate. Playback snapshots the Job revision; Job switch and mutations are blocked while the playback session is `playing` or `paused`. Only explicit Stop returns to `idle` and unlocks editing.
- OPC UA Joint source makes Job editing and playback read-only without hiding saved Jobs.
- Project snapshot is authoritative. Legacy `robot-sim.pose-sequence.v1` localStorage is one-time recovery input only and is deleted after successful Project commit.
...
- Generated Objects support name, dimensions, color, Graspable default OFF, manual MCP-local XYZRPY, numeric status overlay, OPC binding, persistence, and deletion. This stage has no separate per-Object Manual frame selector; only OPC bindings select World or MCP, with MCP as the default. V1/V2 instances also migrate with Graspable OFF because that choice was not durable in the legacy Project format.
...
- BAD, STALE, disconnect, missing, invalid, duplicate, and out-of-order data hold the last accepted trajectory/pose and never apply a zero or partial Pose.
- Wire `GOOD | BAD` quality and browser runtime `WAITING | STALE | DISCONNECTED` state remain separate; STALE uses browser receipt time. Binding/source/generation changes clear old samples, and an entity generation token rejects late frames after rebinding or deletion.
- OPC ownership blocks manual numeric edits, drag, and grasp. Returning to Manual restores the last saved manual fallback instead of the live OPC Pose.
- An accepted target updates collision/report content revision only when the resulting target matrix changes. Per-frame interpolation does not churn revisions.
...
- One unique STEP source hash produces exactly one archive STEP entry, including a one-source seven-Link Robot.
...
- Corrupt input, missing references, revision mismatch, parser failure, or any budget breach leaves the active revision, scene, selection, repositories, and collision registry unchanged.
- Fault injection at DB revision write, active-pointer swap, cache publish, and runtime publish produces only a complete old revision or complete new revision, never a mixed state.
- Exact Job limits 32/256/2,048 pass; plus one fails before mutation.

### SC-ROBOT: deterministic assembly import

- One through seven unique sources resolve exactly `LINK00` through `LINK06`, with no missing or duplicate mesh ownership.
- Each source is read, hashed, parsed, and stored once. Reused source references do not duplicate bytes.
- Local acceptance fixture: 13,093,130 bytes, SHA-256 `4130e05b6287fa47a49d376b6ab3cde3c98306155118d6f6e06751d1067b9ef1`, 38,299 triangles, seven named Link meshes, and occt-import-js 0.0.23. Keep the file local; CI uses a synthetic assembly-tree fixture.
...
- Datasheet mechanics load as the default; finite user edits to Base pose, Joint origin/axis/limits/maximum velocity, and Link geometry-local transforms round-trip exactly, while invalid edits and geometry remapping leave the prior mechanics unchanged.
...
- Job switch does not move the Robot. Playback snapshots one active Job and blocks Job/Pose mutation until stop.
...
- Creation of one Asset plus one Instance is atomic. Invalid input and failed delete leave state unchanged.
- Save/reload/export/import preserves source kind, dimensions, color, Graspable, canonical MCP-local Manual transform, overlay, numeric status, collision proxy, and OPC binding/reference exactly.
...
- STALE is `max(3 * samplingIntervalMs, 1000 ms)`. BAD/STALE/bounded malformed-string protocol fault/disconnect samples and freezes the current effective Pose, marks motion `HELD`, and never applies a zero or partial Pose. Malformed strings refresh no clocks; binary/oversized input closes the connection as DISCONNECTED.
- Background-tab resume clears obsolete trajectory time. It snaps once only to a fresh same-generation target that remains current-quality GOOD with no later non-GOOD/fault; an absent/stale/BAD/faulted target keeps the current Pose HELD until the first later fresh complete GOOD establishes the baseline.
...
- Connector limits: 32 transform profiles, 256 total read Nodes, 65,536 UTF-8 bytes per catalog/frame payload, 128 UTF-8 bytes per gateway/profile/display id, and 1,024 UTF-8 bytes per NodeId. Exact boundary passes; plus one fails before startup or state reduction.
...
- `MANUAL`, `SIMULATION`, `OPC UA`, `GOOD`, `UNCERTAIN`, `BAD`, `WAITING`, `STALE`, `DISCONNECTED`, and `HELD` use text plus icon, never color alone.
- Keyboard-only operation completes mode switch, Add/import, Job CRUD/reorder/playback, binding, and validation with no keyboard trap and correct dialog focus return.
...
- [ ] **Gate R3: Middleware protocol**
  ...
  Expected: joint, numeric-status, transform-profile, atomic-frame, size-limit, reconnect, and stale-quality tests pass.
...
- [ ] **Gate R5: Browser workflows**

  Run: `npm run test:e2e`

  Expected: existing Project round-trip and geometry-collision tests plus new assembly import, Simulation Job, primitive Object, OPC transform/smoothing, and four-mode workspace tests pass with no timeout waiver.
...
- [ ] **Gate R6: Deployment compatibility**
  ...
  Expected: deployment config validates, images build, web health smoke passes, and Connector-enabled smoke receives a valid protocol frame without OPC write capability.
...
| M3 | Simulation Jobs | Create/duplicate/reorder/play/validate/reload | WS3 criteria pass |
