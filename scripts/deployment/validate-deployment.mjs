...
export function validateDeploymentContract(files) {
  ...
  requirePattern(errors, files.nginx, /location\s*=\s*\/runtime\/ws/, 'Nginx must expose the exact Runtime Gateway WebSocket route.')
  requirePattern(errors, files.nginx, /proxy_set_header\s+Upgrade\s+\$http_upgrade\s*;/i, 'Nginx WebSocket proxy must forward the Upgrade header.')
  requirePattern(errors, files.nginx, /proxy_set_header\s+Connection\s+["']upgrade["']\s*;/i, 'Nginx WebSocket proxy must forward the Connection upgrade token.')
