...
describe('AppMenuBarV4', () => {
  ...
  it('uses roving top-level and nested keyboard navigation', async () => { render(<Harness />); const project = screen.getByRole('menuitem', { name: 'Project' }); fireEvent.keyDown(project, { key: 'ArrowRight' }); expect(screen.getByRole('menuitem', { name: 'View' })).toHaveFocus(); fireEvent.keyDown(screen.getByRole('menuitem', { name: 'View' }), { key: 'ArrowDown' }); fireEvent.keyDown(screen.getByRole('menuitem', { name: 'View' }), { key: 'ArrowLeft' }); expect(screen.getByRole('menu', { name: 'Project' })).toBeInTheDocument(); fireEvent.keyDown(screen.getByRole('menuitem', { name: 'Samples' }), { key: 'ArrowRight' }); await Promise.resolve(); expect(screen.getByRole('menu', { name: 'Samples' })).toBeInTheDocument(); fireEvent.keyDown(screen.getByRole('menu', { name: 'Samples' }), { key: 'ArrowLeft' }); expect(screen.getByRole('menuitem', { name: 'Samples' })).toHaveFocus() })
