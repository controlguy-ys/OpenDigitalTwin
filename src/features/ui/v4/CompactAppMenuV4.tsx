...
export function CompactAppMenuV4({ commandBindings, model, openSection, onOpenSectionChange, onPreviewSection }: CompactAppMenuPropsV4): ReactNode {
  const root = useRef<HTMLDivElement>(null); const disclosure = useRef<HTMLButtonElement>(null); const callback = useRef(onPreviewSection); const categoryTrigger = useRef<HTMLButtonElement | null>(null)
  ...
  useEffect(() => { callback.current(openSection) }, [openSection])
  ...
  const backToCategories = (restore = true): void => { setSubmenu(null); onOpenSectionChange(null); if (restore) queueMicrotask(() => categoryTrigger.current?.focus()) }
  ...
  const renderNodes = (items: readonly AppMenuNodeV4[]): ReactNode => items.map((node) => {
    ...
    return <div key={node.id} className="app-menu-submenu-v4"><button id={triggerId} type="button" role="menuitem" data-menu-direct="true" aria-haspopup="menu" aria-expanded={open} aria-controls={submenuId} onClick={() => setSubmenu(open ? null : node.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ' || event.key === 'ArrowRight') { event.preventDefault(); event.stopPropagation(); setSubmenu(node.id); queueMicrotask(() => { const popup = document.getElementById(submenuId); if (popup) focus(popup, 'first') }) } }}>{node.label}<span className="app-menu-submenu-indicator-v4" aria-hidden="true" /></button>{open ? <div id={submenuId} role="menu" aria-labelledby={triggerId} onKeyDown={(event) => { if (event.key === 'ArrowLeft') { event.preventDefault(); event.stopPropagation(); setSubmenu(null); document.getElementById(triggerId)?.focus() } else if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); close(true) } else if (event.key === 'ArrowDown') { event.preventDefault(); event.stopPropagation(); focus(event.currentTarget, 1) } else if (event.key === 'ArrowUp') { event.preventDefault(); event.stopPropagation(); focus(event.currentTarget, -1) } else if (event.key === 'Home') { event.preventDefault(); event.stopPropagation(); focus(event.currentTarget, 'first') } else if (event.key === 'End') { event.preventDefault(); event.stopPropagation(); focus(event.currentTarget, 'last') } }}>{renderNodes(node.children)}</div> : null}</div>
  })
  return <div ref={root} className="compact-app-menu-v4"><button ref={disclosure} type="button" aria-haspopup="menu" aria-expanded={expanded} aria-controls={rootId} onClick={() => { const next = !expanded; setExpanded(next); if (!next) onOpenSectionChange(null); else queueMicrotask(() => { const popup = document.getElementById(rootId); if (popup) focus(popup, 'first') }) }}>Menu</button>{expanded ? <div id={rootId} role="menu" aria-label="Application menu" className="app-menu-popup-v4" onKeyDown={(event) => navigate(event, () => undefined)}>{section === null ? model.map((item) => { const categoryId = `${id}-${item.id}-category`; return <button key={item.id} ref={(node) => { if (item.id === openSection) categoryTrigger.current = node }} id={categoryId} type="button" role="menuitem" data-menu-direct="true" aria-haspopup="menu" aria-expanded="false" aria-controls={`${id}-${item.id}-section`} onClick={() => { categoryTrigger.current = document.getElementById(categoryId) as HTMLButtonElement; onOpenSectionChange(item.id); queueMicrotask(() => { const popup = document.getElementById(`${id}-${item.id}-section`); if (popup) focus(popup, 'first') }) }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ' || event.key === 'ArrowRight') { event.preventDefault(); categoryTrigger.current = event.currentTarget; onOpenSectionChange(item.id); queueMicrotask(() => { const popup = document.getElementById(`${id}-${item.id}-section`); if (popup) focus(popup, 'first') }) } }}>{item.label}<span className="app-menu-submenu-indicator-v4" aria-hidden="true" /></button> }) : <div id={`${id}-${section.id}-section`} role="menu" aria-label={section.label} onKeyDown={(event) => navigate(event, () => backToCategories(true))}>{renderNodes(section.children)}</div>}</div> : null}</div>
}
