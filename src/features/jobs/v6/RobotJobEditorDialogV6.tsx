...
function EditorContent({ project, job, authoring, onClose, triggerRef, runtimeState }: RobotJobEditorDialogV6Props & { readonly job: WorkcellProjectV5['jobs'][number]; readonly runtimeState: RobotJobRuntimeStateV5 | undefined }): ReactNode {
  ...
  return <ModalDialogV6 className="v6-job-editor-dialog" footer={<footer className="v6-job-editor-footer"><button onClick={onClose} type="button">Close</button></footer>} header={<header className="v6-job-editor-header"><h2 id="v6-job-editor-title">Edit Job: {job.name}</h2><p>{running ? 'A running Job is read-only.' : 'Changes publish one Project revision per action.'}</p></header>} onClose={onClose} size="editor" titleId="v6-job-editor-title" {...(triggerRef === undefined ? {} : { triggerRef })}>
    <div className="v6-job-editor-body"><JobInstructionListV6 disabled={running} instructions={job.instructions} onReorder={(id, beforeId) => invoke(() => authoring.reorder(job.id, id, beforeId))} onSelect={setSelectedId} project={project} selectedInstructionId={selectedId} state={runtimeState?.state ?? 'IDLE'} stepIndex={runtimeState?.stepIndex ?? null} />
      <aside aria-label="Step Inspector" className="v6-job-step-inspector"><h3>Step Inspector</h3>{selected === null ? <p>Select a step.</p> : <><p><strong>{selected.kind}</strong></p><p>{jobInstructionSummaryV6(selected, project)}</p><label>Instruction kind<select aria-label="Instruction kind" disabled={running} onChange={(event) => setKind(instructionKind(event.currentTarget.value))} value={draft.kind}><option value="move-joint">Move joint</option><option value="set-do">Set DO</option><option value="wait-di">Wait DI</option><option value="delay">Delay</option><option value="attach">Attach</option><option value="detach">Detach</option></select></label>
        ...
        {draftError !== null && <p role="alert">{draftError}</p>}<div className="v6-job-context-actions"><button disabled={running} onClick={replace} type="button">Edit</button><button disabled={running} onClick={() => insert(selected.id)} type="button">Insert Before</button><button disabled={running} onClick={() => insert(nextInstructionId)} type="button">Insert After</button><button disabled={running} onClick={() => invoke(() => authoring.duplicate(job.id, selected.id))} type="button">Duplicate</button><button disabled={running} onClick={() => invoke(() => authoring.remove(job.id, selected.id))} type="button">Delete</button><button disabled={running || index <= 0} onClick={() => invoke(() => authoring.reorder(job.id, selected.id, job.instructions[index - 1]?.id ?? null))} type="button">Move Before</button><button disabled={running || index < 0 || index >= job.instructions.length - 1} onClick={() => invoke(() => authoring.reorder(job.id, selected.id, job.instructions[index + 2]?.id ?? null))} type="button">Move After</button></div></>}</aside></div>
  </ModalDialogV6>
}
